export const About = () => {
  return (
    <div
      id="about"
      className="max-w-5xl px-6 pt-20 pb-30 mx-auto space-y-4 font-medium [&_section]:pb-4 prose divide-y dark:prose-invert md:px-0"
    >
      <section>
        <h2>About Talor</h2>
        <p>
          <b className="font-semibold text-inter text-foreground">Talor</b> is a
          simple application designed to help people{' '}
          <b className="font-semibold text-inter text-foreground">
            write, organize, and share their thoughts, history.
          </b>{' '}
          It allows users to create personal notes or Talor entries, keep them{' '}
          <b className="font-semibold text-inter text-foreground">private</b>,
          or{' '}
          <b className="font-semibold text-inter text-foreground">
            publish them
          </b>{' '}
          for others in the community to read
        </p>
        <p>
          The goal of Talor is to provide a{' '}
          <b className="font-semibold text-inter text-foreground">
            calm and simple space
          </b>{' '}
          where ideas, memories, and daily reflections can be written and
          preserved.
        </p>
      </section>

      <section>
        <h2>What You Can Do</h2>
        <ul>
          <li>
            <p>
              <b className="font-semibold text-inter text-foreground">
                Write Notes{' '}
              </b>
              - capture ideas, thoughts, or daily experiences
            </p>
          </li>
          <li>
            <p>
              <b className="font-semibold text-inter text-foreground">
                Organize Your Thoughts{' '}
              </b>
              - capture ideas, thoughts, or daily experiences
            </p>
          </li>
          <li>
            <p>
              <b className="font-semibold text-inter text-foreground">
                Keep Notes Private{' '}
              </b>
              - capture ideas, thoughts, or daily experiences
            </p>
          </li>
          <li>
            <p>
              <b className="font-semibold text-inter text-foreground">
                Publish to the Community{' '}
              </b>
              - capture ideas, thoughts, or daily experiences
            </p>
          </li>
          <li>
            <p>
              <b className="font-semibold text-inter text-foreground">
                Discover Stories{' '}
              </b>
              - capture ideas, thoughts, or daily experiences
            </p>
          </li>
        </ul>
      </section>

      <section>
        <h2>Our Vision</h2>
        <p>
          Talor aims to create a
          <b className="font-semibold text-inter text-foreground">
            digital space for ideas and reflections.
          </b>
        </p>
        <p>
          Rather than being a noisy social platform, it focuses on providing a{' '}
          <b className="font-semibold text-inter text-foreground">
            quiet place to write, think, and share meaningful content.
          </b>
        </p>
      </section>

      <section>
        <h2>Our Principles</h2>
        <p>Talor is built around a few simple priciples: </p>
        <ul>
          <li>
            <p>
              <b className="font-semibold text-inter text-foreground">
                Simplicity{' '}
              </b>
              - capture ideas, thoughts, or daily experiences
            </p>
          </li>
          <li>
            <p>
              <b className="font-semibold text-inter text-foreground">
                Privacy{' '}
              </b>
              - capture ideas, thoughts, or daily experiences
            </p>
          </li>
          <li>
            <p>
              <b className="font-semibold text-inter text-foreground">
                Freedom to Share{' '}
              </b>
              - you decide whether your notes stay private or become public
            </p>
          </li>
        </ul>
      </section>

      <section>
        <h2>Community</h2>
        <p>
          Talor encourages a community where people can{' '}
          <b className="font-semibold text-inter text-foreground">
            share experiences, ideas, and reflections.
          </b>
        </p>
        <p>
          Published notes allow users to connect through stories, thoughts, and
          perspectives from others.
        </p>
      </section>
    </div>
  );
};
